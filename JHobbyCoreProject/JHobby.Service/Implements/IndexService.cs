...
namespace JHobby.Service.Implements
{
    public class IndexService : IIndexService
    {
        ...
        public IEnumerable<QueryMemberGenderModel> GetGenderResult()
        {
            ...
            var _total = resporitory.Where(res => res.Status == "1").Count();
            var _man = resporitory.Where(res => res.Gender == "1" && res.Status != "9").Count();
            var _woman = resporitory.Where(res => res.Gender == "0" && res.Status != "9").Count();
